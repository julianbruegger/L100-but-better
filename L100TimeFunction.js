document.addEventListener('click', function () {
    // Code inside this function will be executed when a click event occurs on the document.
    // You can place your desired actions or logic here.
    ...
    Array.from(updatedDetailGrid.children).forEach((y) => {
        ...
        if (absoluteMinutes < 10) {
            y.innerHTML = `${absoluteHours}:0${absoluteMinutes}h (${hours}.${minutes})`;
            y.style = "text-align: left;"
        }
        else {
            y.innerHTML = `${absoluteHours}:${absoluteMinutes}h (${hours}.${minutes})`;
            y.style = "text-align: left;"
        }
